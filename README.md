...
Version 0.3.0 introduced PluginKit v1. Bundled plugins are packaged with versioned manifests and signed with the app. The settings window can import, inspect, enable, disable, update, and remove `.codexstatusplugin` resource packs. Imported packages never execute third-party code: executable files, symbolic links, incompatible host versions, and attempts to replace bundled plugins are rejected. The complete package contract is documented in [`Documentation/PLUGIN_SPEC.md`](Documentation/PLUGIN_SPEC.md), with a machine-readable manifest schema beside it. The completed 0.3 foundation is recorded in [`Documentation/ROADMAP_0.3.md`](Documentation/ROADMAP_0.3.md).

Version 0.3.1 added the Prompt & Constraint Library, direct verified update downloads, single-instance protection, and adaptive menu-bar width. Version 0.3.2 added permission preflight and the corrected native settings-window toolbar.

With permission preflight, plugins declare each protected capability and a user-facing reason. CodexStatus reviews the complete list during import, first enable, or migration from an older enabled build; supported macOS permission prompts are requested in that same flow. A plugin does not start unless every required permission succeeds. Approval remains valid for unchanged declarations, while added or changed permissions force a fresh review. The settings window uses an opaque native toolbar with a stable leading sidebar control in both expanded and collapsed states.
...
Release maintainers should follow the documented [`Documentation/RELEASING.md`](Documentation/RELEASING.md) checklist so every published version includes complete notes, a verified app archive, and its checksum.
