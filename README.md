...
The app has no Dock icon. Look for the colored Codex-shaped mark in the menu bar. On first launch it installs additive user-level Codex lifecycle hooks. Codex may ask you to review and trust the hook definition. The hook only writes status snapshots under `~/Library/Application Support/CodexStatus`; it does not change prompts, approve actions, or send data over the network.

New Codex activity then appears automatically. The app reads the persisted turn lifecycle, so a task remains blue throughout model thinking and tool work instead of guessing from its last-update time. A completed task remains green until it is opened from CodexStatus; that acknowledgement is persisted across app restarts.

Ordinary idle tasks are grouped into a collapsed row to keep the popover compact. Unread completed tasks remain visible until opened, then join the idle group. The idle row can be expanded whenever older tasks are needed.
...
A completed task remains green until it is seen. Moving the pointer over the completed task row marks it as read immediately; it becomes a neutral `Completed · viewed` row, remains visible, and the acknowledgement is persisted.

CodexStatus installs a lightweight user LaunchAgent watcher. The watcher has no UI and only observes whether the Codex app bundle is running: it opens CodexStatus when Codex starts and terminates CodexStatus when Codex quits.

## Privacy

CodexStatus runs locally. It reads Codex task metadata and rate-limit information from the local Codex App Server and stores only small lifecycle snapshots and completion acknowledgements on your Mac. CodexStatus itself does not upload analytics or user data.
