...
while IFS= read -r message; do
    ...
    case "$method" in
        ...
        thread/list)
            ...
            state_db_only=$(print -r -- "$message" | jq -r '.params.useStateDbOnly // false')
            if [[ "$state_db_only" != "true" ]]; then
                print -r -- "{\"id\":$request_id,\"error\":{\"message\":\"background scanner must avoid transcript repair\"}}"
                continue
            fi
